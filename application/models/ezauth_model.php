...
/*
	EzAuth is used to manage a database of users on your website. 
	You can protect certain pages of your website, giving users access keys to the different portions of your website. 
	Users can also have administrator access and universal keys to gain access to any section on your domain. 
	You can also define custom key definitions outside of EzAuth that integrate with EzAuth.
...
var $user		 			=	array();	//	user info in database
	...
	function login($un = null, $pw = null, $give_new_key = false, $cookie_hash = '', $get_all_user_info = true) {
		...
		if (empty($cookie_hash)) {
			...
			$query = $this->db->get_where('ez_users', array('lower(username)' => strtolower($un), 'password' => $this->_add_salt($pw)));
		} else {
